...
function allowedFile(filename) {
  return filename.split('.').pop().toLowerCase() === 'xlsx'||filename.split('.').pop().toLowerCase() === 'csv'|| filename.split('.').pop().toLowerCase() === 'xls' || filename.split('.').pop().toLowerCase() === 'txt';
}
   ...
   //  Decode range and start at Excel row 7 (0-based index 6)
    ...
    data = XLSX.utils.sheet_to_json(workbook.Sheets["Sheet1"], { header: 1 });
     ...
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
      ...
      fullRange.s.r=findHeaderRow(sheet);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }else if(supplierName==="BUGATTI_GMBH"){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }else if(supplierName==="ONLINE_TEXTILE_SOLUTIONS_A/S"|| supplierName === "BERG_BRAND_MANAGEMENT_ApS" || supplierName === "PYRENEX_SAS" || supplierName === "FB-TOOLS_OY"){
      const sheet = workbook.Sheets[sheetName];
      const fullRange = XLSX.utils.decode_range(sheet['!ref']);
      fullRange.s.r=findHeaderRow(sheet);
      console.log(fullRange.s+"--"+supplierName+" header "+fullRange.s.r);
     data = XLSX.utils.sheet_to_json(sheet, { header: 1, range: fullRange });
    }
 else {
